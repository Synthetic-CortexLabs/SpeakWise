...
class Feedback(models.Model):
    """
    Feedback Model
    This model represents feedback given by a reviewer for a specific talk at
    an event.
    It includes ratings on various aspects such as speaker expertise, depth
    of topic, relevancy, and value or impact, as well as optional comments.
    Attributes:
        NUM_STARS (list of tuples): Choices for rating fields, ranging from 1
          to 5.
        talk (ForeignKey): Reference to the Talks model, representing the talk
           being reviewed.
        event (ForeignKey): Reference to the Event model, representing the
        event where the talk was given.
        reviewer (ForeignKey): Reference to the User model, representing the
        reviewer.
        speaker_expertise (IntegerField): Rating for the speaker's expertise,
        with choices from NUM_STARS.
        depth_of_topic (IntegerField): Rating for the depth of the topic, with
        choices from NUM_STARS.
        relevancy (IntegerField): Rating for the relevancy of the talk, with
        choices from NUM_STARS.
        value_or_impact (IntegerField): Rating for the value or impact of the
        talk, with choices from NUM_STARS.
    ...
    Meta:
        ...
        verbose_name_plural (str): Human-readable name for the model in plural
        form.
    Methods:
        __str__(): Returns a string representation of the feedback, including
        the reviewer's username,
                   the talk title, and the event title.
        average_score(): Calculates and returns the average score of the
        ratings.
                         Returns None if the total score is zero or if an
